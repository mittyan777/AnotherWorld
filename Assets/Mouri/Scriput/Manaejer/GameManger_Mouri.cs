...
public class GameManger_Mouri : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Status[4] == 1) { job_text.text = ($"åïém"); }
        if (Status[4] == 2) { job_text.text = ($"ÉAÅ[É`ÉÉÅ["); }
        if (Status[4] == 3) { job_text.text = ($"äiì¨â∆ "); }
        if (Status[4] == 4) { job_text.text = ($"É}ÉWÉVÉÉÉì "); }
    ...
    public void stopslot()
    {
        ...
        // êEéÌñºÇçÏÇÈ
        string jobName = "";
        if (Status[4] == 1) jobName = "åïém";
        else if (Status[4] == 2) jobName = "ã|égÇ¢";
        else if (Status[4] == 3) jobName = "ñÇñ@";

        // Chang Ç…âÊëúêÿë÷Çì`Ç¶ÇÈ
        FindObjectOfType<Chang>().jobName(jobName);
