...
public class Player_main : MonoBehaviour
{
    ...
    void Archercontrol()
    {
        if (UnityEngine.Input.GetMouseButton(0) && mouseleft == false)
        {
            ...
            {
                GetComponent<ArcherÅQskill_main>().NormalAttack();
            }
            mouseleft = true;
        ...
        {
            mouseright = false;
            GetComponent<ArcherÅQskill_main>().shootpower = 0;
        }

        if (mouseright == true && mouseleft == true)
        {
            if (GetComponent<ArcherÅQskill_main>().shootpower <= 30)
            {
                GetComponent<ArcherÅQskill_main>().shootpower += 10 * Time.deltaTime;
            }
        }
