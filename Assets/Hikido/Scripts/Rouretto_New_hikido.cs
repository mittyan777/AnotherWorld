...
public class Rouretto_New_hikido : MonoBehaviour
{
    ...
    [Header("職種ごとの画像設定")]
    [SerializeField] private Rouletto_CG_main_hikido rouletto_CG;
    ...
    void Start()
    {
        ...
        if (rouletto_CG == null)
        {
            rouletto_CG = FindAnyObjectByType<Rouletto_CG_main_hikido>();//職種ごとに画像を変えるための部分
        }
    ...
    private void PlayerStatus()
    {
        ...
        GameManager.GetComponent<GameManager_hikido>().Status[0] = HP;
        GameManager.GetComponent<GameManager_hikido>().Status[1] = MP;
        GameManager.GetComponent<GameManager_hikido>().Status[2] = Attack;
        GameManager.GetComponent<GameManager_hikido>().Status[3] = Defense;
        GameManager.GetComponent<GameManager_hikido>().job = job;
    }
    ...
    public void StartRoulette()
    {
        ...
        {
            if (GameManager.GetComponent<GameManager_hikido>().Coin >= RouletteCost)   //GameManegerの中にあるコインがルーレットを回すために必要なコインより多かった場合
            {
                Spining = true;
                spinCoroutine = StartCoroutine(SpinRoulette());
                GameManager.GetComponent<GameManager_hikido>().Coin -= RouletteCost;
    ...
    void Status_Boost()
    {
        GameManager.GetComponent<GameManager_hikido>().HP += GameManager.GetComponent<GameManager_hikido>().Status[0];
        GameManager.GetComponent<GameManager_hikido>().MP += GameManager.GetComponent<GameManager_hikido>().Status[1];
        GameManager.GetComponent<GameManager_hikido>().AttackStatus += GameManager.GetComponent<GameManager_hikido>().Status[2];
        GameManager.GetComponent<GameManager_hikido>().DefenseStatus += GameManager.GetComponent<GameManager_hikido>().Status[3];
        GameManager.GetComponent<GameManager_hikido>().job = job;
